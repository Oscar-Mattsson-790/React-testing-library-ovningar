// ShakespeareInsultGenerator.jsx
...
export default function ShakespeareInsultGenerator() {
  const [insult, setInsult] = useState(null);

  const generateInsult = () => {
    const randomIndex = Math.floor(Math.random() * insultArray.length);
    setInsult(insultArray[randomIndex]);
  };

  return (
    <div>
      <button onClick={generateInsult}>Insult me!</button>
      {insult ? (
        <p>
          {insult.insult} <em>{insult.play}</em>
        </p>
      ) : (
        <p>"Awaiting insult..."</p>
      )}
